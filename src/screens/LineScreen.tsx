      ...
      {loading ? (<ActivityIndicator size="large" color="black" />) :
        (
          notDisponible ? (<Text style={styles.error}>Données indiponibles</Text>) : (<View style={styles.container}>
            ...
            <DropdownComponent data={[...data.map((v, i) => {
              return {
                label: (
                  <>
                    <Text>
              ...
              onChange={(v) => {
                setDestIndex(v); // console.log(data[destIndex]);
              }} />
            ...
            <FlatList
              ...
              renderItem={({ item }) => (
                <LineTimesCard
                  ...
                  onPress={() => navigation.push("stop", { type: "stop", name: item.parentStation.name, color: "white", textColor: "black", code: item.parentStation.code, line: currentCode})}
                />
              )}
              refreshControl={
